...
def create_transition(*, from_state: Optional[StateDefinition], to_state: StateDefinition, requires_signature: bool):
    ...
    return Transition.objects.create(
        from_state=from_state, to_state=to_state, requires_signature=requires_signature
    )
...
def create_transition_request(
    *,
    ...
    if (not user_instance_permission
        or not user_instance_permission.transactions.filter(
            id=transition.id
        ).exists()
    ):
    ...
    if TransitionRequest.objects.filter(
        is_obsolete=False,
        content_type=content_type,
        object_id=object_id
    ).exists():
